...
async def _interaction_state(page) -> dict:
    ...
    try:
        return await page.evaluate(
            """() => {
              // Compare the whole body, not just <main>: modals, toasts, and
              // dropdowns are frequently portaled as direct body children and
              // would otherwise be invisible to this diff.
              const root = document.body;
              const text = String(root?.innerText || '').replace(/\\s+/g, ' ').slice(0, 4000);
              const controls = Array.from(document.querySelectorAll(
                'button, a[href], input, textarea, select, [role="button"], [role="tab"], ' +
                '[role="switch"], [role="dialog"], [role="alertdialog"], [aria-modal="true"]'
              )).slice(0, 80).map((el) => [
              ...
              // Count of open dialogs/modals catches portaled UI even if its
              // text content happens to fall outside the 4000-char slice.
              const modal_count = document.querySelectorAll(
                '[role="dialog"], [role="alertdialog"], [aria-modal="true"]'
              ).length;
              return {
                url: window.location.href, text, controls,
                scroll_y: Math.round(window.scrollY), modal_count,
...
def _state_changed(before: dict, after: dict, expected_result: object = "") -> bool:
    ...
    if abs(int(before.get("scroll_y", 0)) - int(after.get("scroll_y", 0))) > 8:
        return True
    if int(before.get("modal_count", 0)) != int(after.get("modal_count", 0)):
        return True
    # A DOM node-count delta catches structural changes (new list items,
    # inserted cards, a mounted panel) whose text/controls happen to fall
    # outside the slices/limits above.
    if abs(int(before.get("node_count", 0)) - int(after.get("node_count", 0))) > 2:
        return True
